...
static void repaint(void){
 int w=ScreenWidth(),h=ScreenHeight(),left=78,right=w-38,top=300,bottom=650,n=count-page*12,i,x,px,py,minT=99,maxT=-99;
 ...
 {int by=970;DrawRect(35,by,w-70,235,BLACK);text(55,by+18,w-110,40,"CO SI VZIT NA SEBE",small,ALIGN_LEFT);text(55,by+68,w-110,140,rec,advice,ALIGN_LEFT|VALIGN_MIDDLE);button(35,1240,185,82,"<");{char s[30];snprintf(s,sizeof s,"%d / %d",page+1,(count+11)/12);text(230,1250,w-460,62,s,body,ALIGN_CENTER);}button(w-220,1240,185,82,">");}
 FullUpdate();
}
static int handler(int type,int p1,int p2){if(type==EVT_INIT){title=OpenFont(DEFAULTFONTB,46,1);body=OpenFont(DEFAULTFONT,32,1);small=OpenFont(DEFAULTFONTB,25,1);tiny=OpenFont(DEFAULTFONT,22,1);advice=OpenFont(DEFAULTFONTB,40,1);load();repaint();}else if(type==EVT_REPAINT)repaint();else if(type==EVT_POINTERUP){int w=ScreenWidth();if(inside(p1,p2,w-270,24,230,75)){refresh();repaint();}else if(inside(p1,p2,35,1230,200,110)&&page>0){page--;repaint();}else if(inside(p1,p2,w-235,1230,220,110)&&(page+1)*12<count){page++;repaint();}}else if(type==EVT_KEYDOWN&&p1==IV_KEY_BACK)CloseApp();else if(type==EVT_EXIT){CloseFont(title);CloseFont(body);CloseFont(small);CloseFont(tiny);CloseFont(advice);}return 0;}
