...
static void fetch_image(Episode*e){char path[300],url[400],cmd[1400];int rc;if(!e->image[0])return;snprintf(path,sizeof path,ROOT "/images/%s.jpg",e->key);if(!access(path,R_OK))return;snprintf(url,sizeof url,"%s%s",BASE,e->image);remove(strcat(strcpy(cmd,path),".tmp"));rc=DownloadTo(0,url,NULL,cmd,15);if(rc!=NET_OK){snprintf(cmd,sizeof cmd,"/bin/busybox wget -q -T 15 -O '%s.tmp' '%s' && mv '%s.tmp' '%s'",path,url,path,path);system(cmd);}else rename(cmd,path);}
