...
static char player_urls[31][700]; static char *player_playlist[32];
...
static void play_episode(int selected){int ids[5],total=indices(ids),i,n=0,pos=page*5+selected;if(selected<0||selected>=5||pos>=total)return;for(i=0;i<count&&n<31;i++)if(!strcmp(ep[i].feed,feed_ids[feed])){snprintf(player_urls[n],sizeof player_urls[n],"%s",ep[i].audio);player_playlist[n]=player_urls[n];n++;}player_playlist[n]=NULL;if(pos>=n)return;LoadPlaylist(player_playlist);SetPlayerMode(autoplay?MP_CONTINUOUS:MP_ONCE);OpenPlayer();PlayTrack(pos);}
