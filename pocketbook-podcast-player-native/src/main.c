...
static void open_system_audio(void){char msg[380];playback_log("05 exec cramfs music_player.app");execl("/ebrmain/cramfs/bin/music_player.app","music_player.app",AUDIO_TMP,(char*)NULL);playback_log("05 exec bin music_player.app");execl("/ebrmain/bin/music_player.app","music_player.app",AUDIO_TMP,(char*)NULL);playback_log("05 exec cramfs audio_book.app");execl("/ebrmain/cramfs/bin/audio_book.app","audio_book.app",AUDIO_TMP,(char*)NULL);playback_log("05 exec cramfs audio_books.app");execl("/ebrmain/cramfs/bin/audio_books.app","audio_books.app",AUDIO_TMP,(char*)NULL);snprintf(msg,sizeof msg,"06 all discovered audio apps failed errno=%d",errno);playback_log(msg);snprintf(status_text,sizeof status_text,"Audio aplikace selhaly, chyba: %d",errno);repaint();}
