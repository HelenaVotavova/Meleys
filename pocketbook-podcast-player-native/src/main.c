...
static void open_system_audio(void){char path[300],msg[380],*handler=GetFileHandler(AUDIO_TMP);pid_t pid;if(handler&&handler[0]){snprintf(msg,sizeof msg,"05 handler=%s",handler);playback_log(msg);if(handler[0]=='/')snprintf(path,sizeof path,"%s",handler);else snprintf(path,sizeof path,"/ebrmain/bin/%s",handler);}else{playback_log("05 no handler");snprintf(path,sizeof path,"/ebrmain/bin/play.app");}pid=fork();if(pid==0){execl(path,path,AUDIO_TMP,(char*)NULL);execl("/ebrmain/bin/audiobooks.app","audiobooks.app",AUDIO_TMP,(char*)NULL);execl("/ebrmain/bin/play.app","play.app",AUDIO_TMP,(char*)NULL);_exit(127);}if(pid<0){playback_log("06 player fork failed");snprintf(status_text,sizeof status_text,"Prehravac nelze spustit.");repaint();return;}playback_log("06 player process started");CloseApp();}
