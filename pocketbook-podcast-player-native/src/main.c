...
static void play_index(int idx){int rc;char cmd[1800];struct stat st;if(idx<0||idx>=count)return;FillArea(35,78,ScreenWidth()-70,48,WHITE);txt(38,82,ScreenWidth()-76,40,"Stahuji docasnou kopii epizody...",f_small,ALIGN_LEFT);FullUpdate();remove(AUDIO_PART);snprintf(cmd,sizeof cmd,"/bin/busybox wget -q -T 300 -O '%s' '%s' || /usr/bin/wget -q -T 300 -O '%s' '%s' || wget -q -T 300 -O '%s' '%s'",AUDIO_PART,ep[idx].audio,AUDIO_PART,ep[idx].audio,AUDIO_PART,ep[idx].audio);rc=DownloadTo(0,ep[idx].audio,NULL,AUDIO_PART,300);if(rc!=NET_OK)rc=system(cmd);if(rc||stat(AUDIO_PART,&st)||st.st_size<1024||!valid_mp3(AUDIO_PART)){remove(AUDIO_PART);snprintf(status_text,sizeof status_text,"Epizodu se nepodarilo nacist.");repaint();return;}remove(AUDIO_TMP);if(rename(AUDIO_PART,AUDIO_TMP)){snprintf(status_text,sizeof status_text,"Docasny soubor nelze ulozit.");repaint();return;}current_audio=idx;LoadPlaylist(local_playlist);SetPlayerMode(MP_ONCE);OpenPlayer();PlayTrack(0);}
