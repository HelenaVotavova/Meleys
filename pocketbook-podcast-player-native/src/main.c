...
static Episode ep[MAXE]; static int count,view,feed,page,autoplay,current_play=-1; static ifont *f_title,*f_body,*f_small;
...
static void refresh(void){int rc;char cmd[1200],url[300];mkdirs();snprintf(url,sizeof url,"%scatalog.dat",BASE);snprintf(cmd,sizeof cmd,"rm -f '%s.tmp'; (/bin/busybox wget -q -T 30 -O '%s.tmp' '%s' || /usr/bin/wget -q -T 30 -O '%s.tmp' '%s' || wget -q -T 30 -O '%s.tmp' '%s')",DATA,DATA,url,DATA,url,DATA,url);Message(ICON_INFORMATION,"Podcasty","Nacitam nove epizody...",2500);remove(DATA ".tmp");rc=DownloadTo(0,url,NULL,DATA ".tmp",30);if(rc!=NET_OK)rc=system(cmd);if(!rc&&access(DATA ".tmp",R_OK)==0)rename(DATA ".tmp",DATA);else rc=-1;if(rc)Message(ICON_ERROR,"Podcasty","Server neni dostupny. Zkus WiFi vypnout a zapnout.",5000);else if(!load_catalog())Message(ICON_ERROR,"Podcasty","Soubor se stahl, ale katalog je poskozeny.",5000);}
...
static void draw_home(void){int w=ScreenWidth(),i;txt(40,30,w-80,65,"Helciny podcasty",f_title,ALIGN_LEFT);txt(40,100,w-80,42,"Online poslech pres WiFi",f_small,ALIGN_LEFT);for(i=0;i<3;i++)btn(55,170+i*98,w-110,88,feed_names[i],0);btn(55,480,w-110,88,autoplay?"Dalsi dil: ZAPNUTO":"Dalsi dil: VYPNUTO",autoplay);btn(55,578,w-110,88,"OBNOVIT KATALOG",0);}
static void draw_list(void){int w=ScreenWidth(),ids[5],total=indices(ids),shown=total-page*5,i;char s[100];if(shown>5)shown=5;txt(35,25,w-70,55,feed_names[feed],f_title,ALIGN_LEFT);snprintf(s,sizeof s,"Epizody %d-%d z %d",page*5+1,page*5+shown,total);txt(38,82,w-76,38,s,f_small,ALIGN_LEFT);for(i=0;i<shown;i++){Episode*e=&ep[ids[i]];int y=130+i*205;char path[300],meta[80];fetch_image(e);snprintf(path,sizeof path,ROOT "/images/%s.jpg",e->key);ibitmap*bm=LoadJPEG(path,145,145,100,100,1);DrawRect(40,y,145,145,BLACK);if(bm){DrawBitmap(40,y,bm);free(bm);}else remove(path);txt(215,y,w-255,92,e->title,f_body,ALIGN_LEFT|VALIGN_MIDDLE);snprintf(meta,sizeof meta,"%s   %s",e->duration,e->date);txt(215,y+105,w-255,40,meta,f_small,ALIGN_LEFT);DrawLine(40,y+175,w-40,y+175,BLACK);}btn(35,1180,175,80,"ZPET",0);btn(w-420,1180,175,80,"<",0);btn(w-220,1180,175,80,">",0);}
static void play_index(int idx){if(idx<0||idx>=count||!ep[idx].audio[0])return;current_play=idx;SetPlayerMode(MP_ONCE);Message(ICON_INFORMATION,"Podcasty","Spoustim online prehravani...",1500);PlayFile(ep[idx].audio);OpenPlayer();}
