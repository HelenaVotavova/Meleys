...
static void open_system_audio(void){char path[300],msg[380],*handler=GetFileHandler(AUDIO_TMP);if(handler&&handler[0]){snprintf(msg,sizeof msg,"05 exec handler=%s",handler);playback_log(msg);if(handler[0]=='/')snprintf(path,sizeof path,"%s",handler);else snprintf(path,sizeof path,"/ebrmain/bin/%s",handler);}else{playback_log("05 no handler, trying play.app");snprintf(path,sizeof path,"/ebrmain/bin/play.app");}execl(path,path,AUDIO_TMP,(char*)NULL);execl("/ebrmain/bin/audiobooks.app","audiobooks.app",AUDIO_TMP,(char*)NULL);execl("/ebrmain/bin/play.app","play.app",AUDIO_TMP,(char*)NULL);playback_log("06 all player exec calls failed");snprintf(status_text,sizeof status_text,"Prehravac nelze spustit.");repaint();}
