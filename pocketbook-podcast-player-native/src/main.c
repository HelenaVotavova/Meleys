...
static char *local_playlist[]={AUDIO_TMP,NULL};
...
static void play_index(int idx){int rc;char cmd[1900],msg[120];struct stat st;if(idx<0||idx>=count)return;FillArea(35,78,ScreenWidth()-70,48,WHITE);txt(38,82,ScreenWidth()-76,40,"Stahuji docasnou kopii epizody...",f_small,ALIGN_LEFT);FullUpdate();remove(DEBUG_LOG);playback_log("01 download start");remove(AUDIO_PART);snprintf(cmd,sizeof cmd,"(/bin/busybox wget -q -T 300 -O '%s' '%s' || /usr/bin/wget -q -T 300 -O '%s' '%s' || wget -q -T 300 -O '%s' '%s') 2>>'%s'",AUDIO_PART,ep[idx].audio,AUDIO_PART,ep[idx].audio,AUDIO_PART,ep[idx].audio,DEBUG_LOG);rc=system(cmd);snprintf(msg,sizeof msg,"02 download result=%d",rc);playback_log(msg);if(rc||stat(AUDIO_PART,&st)||st.st_size<1024||!valid_mp3(AUDIO_PART)){remove(AUDIO_PART);playback_log("03 invalid audio");snprintf(status_text,sizeof status_text,"Epizodu se nepodarilo nacist.");repaint();return;}snprintf(msg,sizeof msg,"03 valid mp3 bytes=%ld",(long)st.st_size);playback_log(msg);remove(AUDIO_TMP);if(rename(AUDIO_PART,AUDIO_TMP)){playback_log("04 rename failed");snprintf(status_text,sizeof status_text,"Docasny soubor nelze ulozit.");repaint();return;}playback_log("04 local file ready");current_audio=idx;LoadPlaylist(local_playlist);playback_log("05 playlist loaded");SetPlayerMode(MP_ONCE);playback_log("06 mode set");OpenPlayer();playback_log("07 player opened");PlayTrack(0);playback_log("08 track started");}
