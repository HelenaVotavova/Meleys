...
static void refresh(void){int rc;char cmd[1200];mkdirs();snprintf(cmd,sizeof cmd,"rm -f '%s.tmp'; (/bin/busybox wget -q -T 30 -O '%s.tmp' '%scatalog.dat' || /usr/bin/wget -q -T 30 -O '%s.tmp' '%scatalog.dat' || wget -q -T 30 -O '%s.tmp' '%scatalog.dat') && test -s '%s.tmp' && mv '%s.tmp' '%s'",DATA,DATA,BASE,DATA,BASE,DATA,BASE,DATA,DATA,DATA);Message(ICON_INFORMATION,"Podcasty","Nacitam nove epizody...",2500);rc=system(cmd);if(rc)Message(ICON_ERROR,"Podcasty","Server neni dostupny. Zkus WiFi vypnout a zapnout.",5000);else if(!load_catalog())Message(ICON_ERROR,"Podcasty","Soubor se stahl, ale katalog je poskozeny.",5000);}
...
static void fetch_image(Episode*e){char path[300],cmd[1400];if(!e->image[0])return;snprintf(path,sizeof path,ROOT "/images/%s.jpg",e->key);if(!access(path,R_OK))return;snprintf(cmd,sizeof cmd,"(/bin/busybox wget -q -T 15 -O '%s.tmp' '%s%s' || /usr/bin/wget -q -T 15 -O '%s.tmp' '%s%s' || wget -q -T 15 -O '%s.tmp' '%s%s') && mv '%s.tmp' '%s'",path,BASE,e->image,path,BASE,e->image,path,BASE,e->image,path,path);system(cmd);}
static void draw_home(void){int w=ScreenWidth(),i;txt(40,30,w-80,65,"Helciny podcasty",f_title,ALIGN_LEFT);txt(40,100,w-80,42,"Online poslech pres WiFi",f_small,ALIGN_LEFT);for(i=0;i<3;i++)btn(55,185+i*140,w-110,95,feed_names[i],0);btn(55,635,w-110,90,autoplay?"Dalsi dil: ZAPNUTO":"Dalsi dil: VYPNUTO",autoplay);btn(55,755,w-110,85,"OBNOVIT KATALOG",0);}
static void draw_list(void){int w=ScreenWidth(),ids[5],total=indices(ids),shown=total-page*5,i;char s[100];if(shown>5)shown=5;txt(35,25,w-70,55,feed_names[feed],f_title,ALIGN_LEFT);snprintf(s,sizeof s,"Epizody %d-%d z %d",page*5+1,page*5+shown,total);txt(38,82,w-76,38,s,f_small,ALIGN_LEFT);for(i=0;i<shown;i++){Episode*e=&ep[ids[i]];int y=130+i*205;char path[300],meta[80];fetch_image(e);snprintf(path,sizeof path,ROOT "/images/%s.jpg",e->key);ibitmap*bm=LoadJPEG(path,145,145,0,0,1);DrawRect(40,y,145,145,BLACK);if(bm){DrawBitmap(40,y,bm);free(bm);}txt(215,y,w-255,92,e->title,f_body,ALIGN_LEFT|VALIGN_MIDDLE);snprintf(meta,sizeof meta,"%s   %s",e->duration,e->date);txt(215,y+105,w-255,40,meta,f_small,ALIGN_LEFT);DrawLine(40,y+175,w-40,y+175,BLACK);}btn(35,1180,175,80,"ZPET",0);btn(w-420,1180,175,80,"<",0);btn(w-220,1180,175,80,">",0);}
