const people=[
 ['Jessica Meir','us','USA','49 let','2026-02-14','jessica','nasa','NASA'],
 ['Jack Hathaway','us','USA','přibl. 43 let','2026-02-14','jack','nasa','NASA'],
 ['Sophie Adenot','fr','Francie','44 let','2026-02-14','sophie','esa','ESA'],
 ['Andrey Fedyaev','ru','Rusko','45 let','2026-02-14','andrey','roscosmos','Roskosmos'],
 ['Anil Menon','us','USA','49 let','2026-07-14','anil','nasa','NASA'],
 ['Pyotr Dubrov','ru','Rusko','48 let','2026-07-14','pyotr','roscosmos','Roskosmos'],
 ['Anna Kikina','ru','Rusko','42 let','2026-07-14','anna','roscosmos','Roskosmos']];
...
async function updateCharts(){try{const r=await fetch('/api/history');const rows=await r.json();const labels=rows.map(x=>new Date(x.timestamp*1000).toLocaleString('cs-CZ',{day:'numeric',month:'numeric',hour:'2-digit',minute:'2-digit'}));const altitude=rows.map(x=>x.altitude),speed=rows.map(x=>x.velocity/3600);if(!altitudeChart){altitudeChart=new Chart(document.querySelector('#altitudeChart'),{type:'line',data:{labels,datasets:[{data:altitude,borderColor:'#08736a',backgroundColor:'#08736a22',fill:true,pointRadius:0}]},options:chartOptions('Průměrná výška','km')});speedChart=new Chart(document.querySelector('#speedChart'),{type:'line',data:{labels,datasets:[{data:speed,borderColor:'#d44835',pointRadius:0}]},options:chartOptions('Průměrná rychlost','km/s')})}else{for(const [chart,data] of [[altitudeChart,altitude],[speedChart,speed]]){chart.data.labels=labels;chart.data.datasets[0].data=data;chart.update()}}}catch(e){}}
