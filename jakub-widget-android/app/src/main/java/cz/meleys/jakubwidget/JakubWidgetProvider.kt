...
class JakubWidgetProvider : AppWidgetProvider() {
    ...
    private fun update(context: Context, manager: AppWidgetManager, ids: IntArray) {
        ...
        ids.forEach { id ->
            ...
            if (child == null) {
                view.setTextViewText(R.id.widget_date, "Jakubův přehled")
            ...
            val refresh = Intent(context, JakubWidgetProvider::class.java).setAction(ACTION_REFRESH)
    ...
    companion object {
        private const val ACTION_REFRESH = "cz.meleys.jakubwidget.REFRESH"
        fun refreshAll(context: Context) {
            val manager = AppWidgetManager.getInstance(context)
            val ids = manager.getAppWidgetIds(ComponentName(context, JakubWidgetProvider::class.java))
            JakubWidgetProvider().update(context, manager, ids)
        }
    }
    ...
    private fun parseChild(dashboard: JSONObject?): JSONObject? = runCatching {
        ...
        (0 until children.length()).map { children.getJSONObject(it) }
            .firstOrNull {
                ...
                name.contains("Jakub", true) || name.contains("Kuba", true)
            } ?: children.optJSONObject(0)
    }.getOrNull()
