...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(state: Bundle?) {
        ...
        findViewById<Button>(R.id.logout).setOnClickListener {
            ...
            JakubWidgetProvider.refreshAll(this)
            showState()
        }
        showState()
    }
    ...
    private fun showState() {
        ...
        status.text = if (signedIn) "" else "Přihlas Jakubovým účtem EduPage."
    ...
    private fun refreshDashboard() {
        ...
        Thread {
            try {
                ...
                store.error = null
                runOnUiThread { status.text = "Aktualizováno"; renderDashboard(); JakubWidgetProvider.refreshAll(this) }
            } catch (error: Exception) {
    ...
    private fun renderDashboard() {
        ...
        val child = (0 until children.length()).map { children.getJSONObject(it) }
            ...
            } ?: children.optJSONObject(0) ?: return
        ...
        objects(rows).forEach { task ->
            val checkbox = CheckBox(this).apply {
                ...
                setOnCheckedChangeListener { _, checked ->
                    ...
                    JakubWidgetProvider.refreshAll(this@MainActivity)
                }
            }
